...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, StdCtrls, Mask, ToolEdit, ExtCtrls, RxLookup, Buttons, Db, DBTables,
  DBFilter, rsDBUtils, ShellApi;
  ...
  Case RadioGroup2.ItemIndex of
    0 : begin
          Case RadioGroup1.ItemIndex of
            0 : begin
                  ...
                  DM1.tEstoqueMatMov.Filter   := 'DtMov >= '''+DateEdit1.Text+''' and DtMov <= '''+DateEdit2.Text+''' and ES = ''E'' and CodMaterial = '''+IntToStr(RxDBLookupCombo1.KeyValue)+''' and CodCor = '''+IntToStr(vCodCor)+'''';
    ...
    1 : begin
          Case RadioGroup1.ItemIndex of
            0 : begin
                  DM1.tEstoqueMatMov.Filtered := False;
                  DM1.tEstoqueMatMov.Filter   := 'DtMov >= '''+DateEdit1.Text+''' and DtMov <= '''+DateEdit2.Text+''' and ES = ''S'' and CodMaterial = '''+IntToStr(RxDBLookupCombo1.KeyValue)+''' and CodCor = '''+IntToStr(vCodCor)+'''';
                  DM1.tEstoqueMatMov.Filtered := True;
                end;
            1, 2 : begin
                  DM1.tEstoqueMatMov.Filtered := False;
    ...
    2 : begin
          Case RadioGroup1.ItemIndex of
            0 : begin
                  DM1.tEstoqueMatMov.Filtered := False;
                  DM1.tEstoqueMatMov.Filter   := 'DtMov >= '''+DateEdit1.Text+''' and DtMov <= '''+DateEdit2.Text+''' and CodMaterial = '''+IntToStr(RxDBLookupCombo1.KeyValue)+''' and CodCor = '''+IntToStr(vCodCor)+'''';
                  DM1.tEstoqueMatMov.Filtered := True;
                end;
            1, 2 : begin
                  DM1.tEstoqueMatMov.Filtered := False;
  ...
  while not DM1.tEstoqueMatMov.Eof do
    begin
      if ((DM1.tEstoqueMatMovTipoMov.AsString = 'NTE') and (CheckBox1.Checked)) or
        ...
        begin
          if ((RadioGroup1.ItemIndex = 2) and (DM1.tEstoqueMatMovlkCodGrupo.AsInteger = RxDBLookupCombo3.KeyValue)) or
...
begin
  ...
  if RadioGroup1.ItemIndex = 1 then
    begin
      if (RxDBLookupCombo1.Text <> '') or (RxDBLookupCombo2.Text <> '') then
  ...
  if RadioGroup1.ItemIndex = 2 then
    begin
      if RxDBLookupCombo3.Text = '' then
        ShowMessage('Falta Informar o Grupo!')
      else
        Le_EstoqueMov;
    end;
...
procedure TfPrevRelHistMaterial.RxDBLookupCombo3Enter(Sender: TObject);
begin
  DM1.tGrupo.IndexFieldNames := 'Nome';
end;

procedure TfPrevRelHistMaterial.RadioGroup1Click(Sender: TObject);
begin
  RxDBLookupCombo1.Color    := clSilver;
  RxDBLookupCombo2.Color    := clSilver;
  RxDBLookupCombo3.Color    := clSilver;
  ...
  if RadioGroup1.ItemIndex = 0 then
    begin
      ...
      RxDBLookupCombo2.Color    := clWindow;
...
procedure TfPrevRelHistMaterial.RadioGroup3Click(Sender: TObject);
begin
  RxDBFilter1.Active := False;
...
procedure TfPrevRelHistMaterial.FormCreate(Sender: TObject);
begin
  oDBUtils.OpenTables(True,Self);
  oDBUtils.ActiveDataSet(True,dm1.tCliente,Name);
  oDBUtils.ActiveDataSet(True,dm1.tFornecedores,Name);
  oDBUtils.ActiveDataSet(True,dm1.tSetor,Name);
