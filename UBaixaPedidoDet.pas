...
var
  fBaixaPedidoDet: TfBaixaPedidoDet;
...
procedure TfBaixaPedidoDet.Grava_Estoque(Qtd : Real);
begin
...
procedure TfBaixaPedidoDet.Grava_PedidoNota(Qtd : Real);
begin
  qUltimoItem.Close;
...
procedure TfBaixaPedidoDet.Baixa_Item(Qtd : Real);
begin
...
procedure TfBaixaPedidoDet.Le_Pedido;
var
  vFlag : Boolean;
  vQtdRestante : Real;
  vQtdFaturada : Real;
begin
  ...
  while not DM1.tPedidoItem.Eof do
  begin
    if StrToFloat(FormatFloat('0',DM1.tPedidoItemQtdParesRest.AsFloat)) > 0 then
      vQtdFaturada := StrToFloat(FormatFloat('0',vQtdFaturada + DM1.tPedidoItemQtdParesFat.AsFloat));
    if StrToFloat(FormatFloat('0',DM1.tPedidoItemQtdParesRest.AsFloat)) > 0 then
      vQtdRestante := StrToFloat(FormatFloat('0',vQtdRestante + DM1.tPedidoItemQtdParesRest.AsFloat));

    DM1.tPedidoItem.Next;
  end;

  DM1.tPedido.Edit;
  DM1.tPedidoQtdParesRest.AsFloat := vQtdRestante;
  if StrToFloat(FormatFloat('0',DM1.tPedidoQtdParesRest.AsFloat)) <= 0 then
    DM1.tPedidoQtdParesRest.AsFloat := StrToFloat(FormatFloat('0',0));
  DM1.tPedidoQtdParesFat.AsFloat  := vQtdFaturada;
  if StrToFloat(FormatFloat('0',DM1.tPedidoQtdParesRest.AsFloat)) > 0 then
    DM1.tPedidoCopiado.AsBoolean := False
  else
    DM1.tPedidoCopiado.AsBoolean := True;
...
procedure TfBaixaPedidoDet.FormClose(Sender: TObject;
...
procedure TfBaixaPedidoDet.BitBtn1Click(Sender: TObject);
begin
  if vOpcao = 'P' then
    Le_Pedido
  else
...
procedure TfBaixaPedidoDet.BitBtn2Click(Sender: TObject);
begin
  Close;
end;

procedure TfBaixaPedidoDet.FormCreate(Sender: TObject);
begin
