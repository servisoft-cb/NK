...
begin
  qPagar.Active := False;
  qPagar.SQL.Clear;
  qPagar.SQL.Add('SELECT DISTINCT DbcPagarparc.CodForn, SUM( DbcPagarparc.RestParcela ) RestParcela, SUM( DbcPagarparc.VlrParcCPagar ) VlrParcCPagar, ');
    ...
    begin
      Case RadioGroup1.ItemIndex of
        0 : begin
              if DateEdit2.Text = '  /  /    ' then
                begin
                  Query1.SQL.Add('WHERE  Dbcpagarparchist.DtUltPgto <= :DtInicial AND Dbcpagarparc.DtVencCPagar >= :DtInicial2 and Dbfornecedores.CodForn = :C1');
                  Query1.Params[0].AsDate := DateEdit1.Date;
                  Query1.Params[1].AsDate := DateEdit1.Date;
                  Query1.Params[2].AsInteger := tMemo1CodForn.AsInteger;;
                end
              else
                begin
                  Query1.SQL.Add('WHERE  Dbcpagarparchist.DtUltPgto <= :DtInicial AND Dbcpagarparc.DtVencCPagar BETWEEN :DtInicial2 AND :DtFinal and Dbfornecedores.CodForn = :C1');
                  Query1.Params[0].AsDate := DateEdit1.Date;
                  Query1.Params[1].AsDate := DateEdit1.Date;
    ...
    begin
      if RadioGroup1.ItemIndex = 0 then
        begin
          if DateEdit2.Text = '  /  /    ' then
            begin
              Query1.SQL.Add('WHERE Dbcpagarparchist.DtUltPgto <= :DtInicial AND dbcpagar.DtGerado <= :DtInicial2 AND dbcpagarparc.DtVencCPagar >= :DtVectoIni and Dbfornecedores.CodForn = :C1');
              Query1.Params[0].AsDate := DateEdit1.Date;
              Query1.Params[1].AsDate := DateEdit1.Date;
            ...
            begin
              Query1.SQL.Add('WHERE  Dbcpagarparchist.DtUltPgto <= :DtInicial AND dbcpagar.DtGerado <= :DtInicial2 AND dbcpagarparc.DtVencCPagar BETWEEN :DtVectoIni AND :DtVectoFin and Dbfornecedores.CodForn = :C1');
              Query1.Params[0].AsDate := DateEdit1.Date;
              Query1.Params[1].AsDate := DateEdit1.Date;
              Query1.Params[2].AsDate := DateEdit1.Date;
